...
**Deployment status:** Implemented, built, and verified locally (migrations run, live smoke tests against real 5SIM/OnlineSIM credentials, `npm run build` clean). Not yet committed/pushed/deployed as of this entry — production `Provider` rows for 5SIM/OnlineSIM still need to be created via the Admin UI post-deploy (only present in the local dev sqlite DB so far), and the backend/frontend deploy to Railway/Vercel for this feature has not been performed yet (the pre-existing Vercel SAML SSO token blocker from §19 remains unresolved).
