...
## 22) Mobile Dashboard UX Polish (10-Point Critique)

User submitted a structured 10-point mobile-dashboard UX critique (spacing, typography, wallet-card layout, currency formatting, status-bar theming, avatar alignment, bottom-nav active color, card interactivity, and quick actions). All 10 addressed:

1. **Side padding** — `AppLayout.tsx`'s `<main>` and `<header>` mobile padding increased `px-4` (16px) → `px-5` (20px); desktop `md:p-8`/`md:px-8` unchanged.
2. **Dashboard heading size** — `dashboard/page.tsx` h1: `text-3xl` → `text-2xl md:text-3xl` (smaller on mobile, unchanged on desktop).
3. **Description de-emphasized** — changed to `text-sm font-normal text-muted-foreground/80` (was `text-muted-foreground` at the default body size/weight).
4. **Wallet card spacing** — extracted the wallet balance out of the generic stat-card grid into its own `Card`, with a larger `size-14` icon (was `size-9`), `gap-4` and `py-2`, and `flex items-center` for vertical centering — instead of being squeezed into a 4-column grid cell.
5. **Consistent 2-decimal currency + visual hierarchy** — `lib/format.ts`'s `formatCurrency` now sets `minimumFractionDigits: 2` (previously only `maximumFractionDigits: 2`, which dropped decimals for whole-number balances). Added a new `splitCurrencyParts()` helper (via `Intl.NumberFormat.formatToParts()`) so the wallet card can render the currency symbol smaller/lighter (`text-lg text-muted-foreground`) next to a larger, bolder amount (`text-3xl font-bold`) — e.g. `₦` + `101.50` instead of one opaque `₦101.50` string.
6. **Edge-to-edge dark status bar** — root cause: Next's `viewport.themeColor` metadata API can only key theme-color off the OS-level `prefers-color-scheme` media query, but this app has its own independent dark/light toggle (`store/themeStore.ts`) that can be set opposite to the OS preference, so a user in dark mode with a light-OS could still get a white Android status bar. Fix: removed the static `viewport.themeColor` array from `app/layout.tsx` and replaced it with a single `<meta name="theme-color" id="theme-color-meta">` tag whose `content` is kept in sync with the *resolved* in-app theme by two call sites that both already run on every theme transition: `ThemeScript.tsx` (pre-hydration, avoids a flash of the wrong color) and `themeStore.ts`'s `applyThemeClass()` (every later toggle/hydration).
7. **Avatar/navbar alignment** — header's right-side `gap-3` → `gap-4` (more breathing room between the theme toggle and the avatar, and between the avatar and the screen edge), and the `Avatar` given `shrink-0` so it can't get compressed under flex pressure.
8. **Bottom-nav active tab not yellow (root-cause CSS bug)** — the active "Home" tab in `BottomNav.tsx` already had the correct `text-primary` class, but rendered navy. Root cause: `globals.css` had a bare, **unlayered** `a { color: inherit; text-decoration: none; }` rule — in CSS cascade layers, *any* unlayered rule always wins over *every* `@layer`-declared rule regardless of selector specificity, so this was silently beating the layered `.text-primary` utility (which lives in Tailwind's `@layer utilities`) on every `<a>` in the app. Fixed by wrapping all of `globals.css`'s base resets in `@layer base { ... }`, restoring Tailwind's own `properties → theme → base → utilities` layer order. Verified via computed style: bottom-nav active color is now `rgb(246, 185, 31)` (`#f6b91f`, brand gold) in both light and dark mode.
9. **Card hover/press interactivity** — stat cards (Orders/Unread notifications/Available services) gained `transition-all hover:-translate-y-0.5 hover:shadow-md hover:ring-primary/20 active:translate-y-0 active:shadow-sm`; the wallet card gained `transition-shadow hover:shadow-md`.
10. **Quick actions below the wallet card** — new 2×2 (mobile) / 1×4 (`sm:`) button grid: **Deposit Funds**, **Withdraw**, **Browse Services**, **View Transactions**, using the codebase's established `Button render={<Link href="..." />}` pattern. No dedicated withdrawal flow exists anywhere in the app (confirmed via a full frontend + backend grep for "withdraw" — zero matches beyond this new button), so both Deposit Funds and Withdraw link to `/wallet` (the closest existing page) rather than a dead link or an invented flow.

**Verified locally before committing** (fresh Playwright + chromium install in-sandbox, mobile 390×844 viewport, both light and dark mode, mocked `/api/*` responses via `page.route()` to sidestep this session's backend CORS restriction to the production Vercel origin only): all computed-style assertions passed — `mainPadding`/`headerPadding` left/right `20px`, `h1FontSize` `24px`, description `14px`/`400` weight, wallet markup `₦` (light) / `101.50` (large, bold), bottom-nav active color `rgb(246, 185, 31)` in both themes, `theme-color-meta` content `#f8fafc` (light) / `#0b1120` (dark) matching `.dark`'s `--color-background`, all four quick-action labels present, all 3 remaining stat cards carrying the new hover/press classes. `npm run build` clean, zero TypeScript errors, all 32 routes.

**Deployment:**
- Committed (`bf734ce`) and pushed to `origin/main`.
- Redeployed to Vercel production: `vercel deploy --prod --yes --token ...` from the monorepo root (same established pattern as §11/§21), using the existing `.vercel/project.json` link. Build restored the prior cache, compiled all 32 routes with zero TypeScript errors in ~31s, aliased to `https://charpsdev.vercel.app`.
- **Live verification**: `GET /`, `/login`, `/settings`, `/support` all `200`; backend `GET /up` still `200`. Re-ran the same Playwright suite directly against `https://charpsdev.vercel.app` (mocked API, logged in as `test@example.com`) — identical results to the local pre-deploy run, confirming all 10 items are live: `mainPadding`/`headerPadding` `20px`, heading `24px`, bottom-nav active color `rgb(246, 185, 31)` in light **and** dark mode, `theme-color-meta` correctly flips `#f8fafc` ↔ `#0b1120` on theme toggle, wallet card shows `₦` + `101.50` with the symbol/amount size split, and all four quick-action buttons present. Screenshots taken in both themes for a final visual sanity check alongside the computed-style assertions.

**Follow-up: `/wallet` page's own balance card also matched to the dashboard's styling.** §22 item #4/#5 was originally scoped to the Dashboard page's wallet card only, since that's what the user's screenshots showed. User then explicitly asked for the dedicated `/wallet` page's separate "Current balance" card (`app/(dashboard)/wallet/page.tsx`) to match too. Applied the identical treatment: `size-14` icon (was `size-12`) with `shrink-0`, `gap-4`/`py-2`/`flex items-center` for more internal padding and vertical centering, switched from `formatCurrency()` to `splitCurrencyParts()` so `₦` renders smaller/muted next to a larger bold `101.50`, matched the page heading/description sizing to the dashboard's mobile treatment, and added the same `hover:shadow-md` transition. Committed (`5e91082`), pushed, redeployed to Vercel production, and re-verified live on `https://charpsdev.vercel.app/wallet` via Playwright (mocked API, 390×844, light + dark) — balance renders as `₦` (small, muted) + `101.50` (large, bold), icon wrapper confirmed `size-14`, dark mode toggle confirmed via the actual `ThemeToggle` UI button. `npm run build` clean, zero TypeScript errors.

## 23) Android App via Capacitor (Remote URL / WebView-shell mode)

User asked to convert the existing Next.js app into an Android app using Capacitor. Confirmed the codebase's architecture first — `frontend/proxy.ts` is Next.js **Middleware** (edge-level cookie-based route protection for `/dashboard`, `/wallet`, `/orders`, `/admin`, etc.) and several pages (`app/page.tsx`, all `(auth)/*` pages) are **Server Components** — both of which are incompatible with Next's static `output: 'export'` mode. This ruled out a fully-bundled offline app without a significant rewrite (converting Middleware to client-side guards, Server Components to Client Components), so proceeded with the lower-risk **"remote URL" / WebView-shell** approach instead: a native Capacitor Android shell whose WebView loads the already-deployed `https://charpsdev.vercel.app` directly, leaving 100% of `frontend/` — auth, routing, SSR — completely untouched.

**New `mobile/` directory** (sibling to `frontend/` and `backend/`, own `package.json`/`node_modules`, independent of the Next.js app's dependency tree):
- `capacitor.config.ts` — `appId: "com.charpsdev.app"`, `appName: "CharpsDev"`, and the key setting: `server.url: "https://charpsdev.vercel.app"`. Also configures `SplashScreen` (`#f8fafc` background, matching the app's light-theme background) and default `StatusBar` (`LIGHT` style / `#f8fafc`).
- `npx cap add android` scaffolded the full native Gradle project under `mobile/android/` (package `com.charpsdev.app`, AGP 8.13, Gradle 8.14.3, `minSdk 24` / `targetSdk 36`, bundled Capacitor plugins `@capacitor/status-bar`, `@capacitor/splash-screen`, `@capacitor/app`).
- **App icon + splash screen** generated via `@capacitor/assets` from the existing `frontend/public/icon-512.png` (paired with a plain `#f8fafc` background layer for the adaptive icon), producing all required mipmap densities (`mdpi` through `xxxhdpi`) plus portrait/landscape/night splash variants.
- **Release signing wired but optional**: `mobile/android/app/build.gradle` reads `android/keystore.properties` if present and signs the `release` build type with it; if absent, it transparently falls back to Android's default debug-signing so `assembleRelease` always succeeds locally even with zero secrets configured (produces a sideload-only APK, not Play-Store-eligible — documented in `mobile/README.md`).
- `mobile/.gitignore` explicitly excludes `keystore.properties`, `*.keystore`/`*.jks`, and all Gradle/Android build output; the root `.gitignore` gained a `mobile/node_modules/` entry.

**`.github/workflows/android-build.yml`** — since this sandbox has no Android SDK/Java (confirmed via `java -version` → command not found before installing a throwaway JDK+SDK locally purely to validate the config, see below), the actual APK build runs in GitHub Actions instead:
- Triggers on push to `main` when `mobile/**` (or the workflow file itself) changes, or manually via `workflow_dispatch` (with an optional checkbox to also build a Play-Store `.aab` bundle alongside the `.apk`).
- JDK 21 + Node 22 + Gradle cache, `npm ci` in `mobile/`, `npx cap sync android`, then `./gradlew assembleRelease`.
- If the four `ANDROID_KEYSTORE_BASE64`/`ANDROID_KEYSTORE_PASSWORD`/`ANDROID_KEY_ALIAS`/`ANDROID_KEY_PASSWORD` repo secrets are set, decodes the keystore and signs a real release build; otherwise builds with the debug-signing fallback and says so plainly in the job summary.
- Uploads the resulting APK (and AAB, if requested) as a workflow artifact, retained 30 days.
- **Note on the GitHub App token boundary:** the sandbox's `setup_github_environment` token cannot create/modify files under `.github/workflows/`, or dispatch/rerun workflow runs (all return `403 Resource not accessible by integration`) — a hard GitHub App permission restriction, not a bug. So the workflow file itself, and every trigger/retry of it, had to be added and clicked manually by the repo owner via GitHub's web UI; the agent could only read run/job status via the API.

**Verification performed in this session (not just "should work"):** since the sandbox lacked Java/Android tooling by default, temporarily installed OpenJDK 21 and a minimal Android SDK (`platform-tools`, `platforms;android-36`, `build-tools;36.0.0`) via `apt`/`sdkmanager` purely to prove the exact same Gradle command CI will run actually succeeds end-to-end against this real config — not left as an unverified assumption:
- `./gradlew assembleRelease --no-daemon` → **BUILD SUCCESSFUL**, produced `app-release.apk` (~6 MB).
- `aapt dump badging` confirmed the manifest: `package: com.charpsdev.app`, `versionName: 1.0`, `targetSdkVersion: 36`, `minSdk: 24`, `application-label: CharpsDev`, `uses-permission: android.permission.INTERNET`.
- `apksigner verify --print-certs` confirmed the APK is validly signed (with the expected Android debug key, since no keystore secrets exist in this sandbox) — i.e. it's a genuinely installable artifact, not a broken build.
- All temporary local build output (`android/app/build/`, `android/build/`, `android/.gradle/`, `android/local.properties`) was deleted before committing — none of it is tracked in git; only source config, the native project skeleton, and generated icon/splash resources are committed.

**CI debugging history (two real config bugs found and fixed via GitHub Actions run logs):**
1. First `workflow_dispatch` run sat `queued` for ~15 min with `runner_id: 0` and zero steps executed, then was auto-cancelled — a transient GitHub-side runner-provisioning delay, not a workflow/build bug (it cleared up on retry).
2. Once a runner was assigned, `npx cap sync android` failed fast: `[fatal] The Capacitor CLI requires NodeJS >=22.0.0` — the workflow was pinned to Node 20. **Fixed:** bumped `actions/setup-node` to `node-version: "22"`.
3. Past that, `./gradlew assembleRelease` failed at `:capacitor-android:compileReleaseJavaWithJavac` with `error: invalid source release: 21` — `capacitor-android` (Capacitor 8) requires Java 21 source/target compatibility, but the workflow's JDK step was pinned to 17 (matching what the in-sandbox verification above had already needed JDK 21 for, in hindsight a signal this was coming). **Fixed:** bumped `actions/setup-java` to `java-version: "21"`.
- Both fixes had to be applied by the user manually via GitHub's web UI (see the GitHub App token boundary note above) using the exact YAML diffs the agent provided in chat.

**Result: ✅ CI build succeeded.** Run [`31129217023`](https://github.com/Clearkess/charpsdev/actions/runs/31129217023) (manual `workflow_dispatch`) and run [`31129756696`](https://github.com/Clearkess/charpsdev/actions/runs/31129756696) (auto-triggered by the JDK-fix push) both completed with `conclusion: success` and produced a `charpsdev-release-apk` artifact (~5.6 MB). Downloaded and independently re-verified the CI-built APK in-sandbox: `aapt dump badging` confirmed `package: com.charpsdev.app`, `application-label: CharpsDev`, `targetSdkVersion: 36`, `minSdk: 24`, `uses-permission: android.permission.INTERNET`; `apksigner verify --print-certs` confirmed a valid signature (Android debug cert, as expected with no release-keystore secrets configured yet).

**`mobile/README.md`** documents: directory layout and the "why remote-URL mode" rationale, local dev commands (`npm run sync` / `open:android` / `build:android`), how to trigger the GitHub Actions build and download the artifact (UI and `gh workflow run` / `gh run download`), full step-by-step real-keystore signing setup (`keytool -genkeypair`, base64-encoding, the four GitHub Secrets to add) with an explicit warning about backing up the keystore file (losing it blocks all future updates to a live Play Store listing under the same signing identity), how to update the app icon/splash/name, and the known limitation that this mode requires an internet connection (no offline support) since it's a live WebView onto the real site rather than a bundled static export.

**Deployment status:** No live "deployment" step applies here in the usual sense — this doesn't touch Vercel or Railway, and there is nothing to "go live" on the web. The deliverable is the CI pipeline itself (`.github/workflows/android-build.yml`) plus the `mobile/` project it builds, and it is now **confirmed working end-to-end**: any push to `mobile/**`/the workflow file, or a manual `workflow_dispatch` from the Actions tab, produces a downloadable, installable `app-release.apk` artifact within a few minutes. Debug-signed for now (installs fine for testing/sideloading); see `mobile/README.md` for adding real release-keystore secrets before a Play Store submission.

**Optional cleanup item (not urgent, user's discretion):** an unrelated stray commit (`f3a64d9`, "Add manual workflow for greeting") added GitHub's generic example `.github/workflows/manual.yml` template while navigating the Actions UI. It doesn't interfere with `android-build.yml` and can be deleted via GitHub's web UI whenever convenient — the agent can't remove it itself due to the same `.github/workflows/` permission restriction.

## 24) Android App Bug: Tapping "Services" Bounced Back to /login

**Symptom (reported after the APK above was confirmed installable):** in the built Android app, logging in worked fine and the dashboard rendered — but the very first tap on the "Services" tab (bottom nav) or sidebar link immediately bounced back to `/login`, every time, right after login. Not intermittent, not tied to idle time.

**Investigation ruled out**, via direct `curl` against production and Playwright-driven Chromium reproductions of the exact login→tap flow (including a "fastest possible click, no delay" variant): the Laravel backend, Sanctum token/session logic, CORS config, and `frontend/proxy.ts`'s own cookie-check logic — all confirmed correct when a valid `charpsdev_token` cookie is actually present on the request. Critically, the bug **could not be reproduced at all in desktop Chromium**, even with a maximally fast click — strongly pointing at something specific to the Android WebView environment rather than a logic bug in the app.

**Root cause (confirmed against a matching Feb 2026 Vercel community bug report of the identical pattern):** Next.js's `<Link>` prefetches protected routes automatically as soon as they scroll into view — including the "Services" tab, right on dashboard load. On a cold Android WebView launch there's a brief window where that automatic prefetch of `/services` can be evaluated by `proxy.ts` middleware *before* the `charpsdev_token` auth cookie (written client-side by `authStore.ts`'s `onRehydrateStorage`, since the source of truth is a `localStorage`-persisted Zustand store, not the cookie itself) has actually committed to the WebView's cookie jar. Middleware sees no cookie on that prefetch request and redirects it to `/login`; Next's router/prefetch cache can then serve that stale cached redirect back the moment the user makes the real tap — even though the cookie is present by then. This lines up with the exact "first tap only" symptom, and with `Bridge.java` (Capacitor's own Android source) never explicitly forcing `CookieManager.setAcceptCookie()`/`setAcceptThirdPartyCookies()`, relying only on Android WebView's own (OEM-inconsistent) cookie-timing defaults.

**Fix (two parts, `commit b26cf61`):**
- **Frontend:** added `prefetch={false}` to every `<Link>` targeting a protected route in `BottomNav.tsx` (the exact "Services" tab reported) and `AppLayout.tsx`'s sidebar, plus the secondary "Browse services" / "Go to profile" / "Track an order" / etc. quick-link buttons on the orders/wallet/cart/settings/support/ErrorScreen pages. This forces every tap to trigger a fresh middleware check instead of risking a cache primed before the cookie existed.
