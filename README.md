...
- **Not yet done**: production `Provider` rows for 5sim/onlinesim (with the real API credentials already verified working in local dev) still need to be created via the Admin UI/API before the feature is actually usable by real users — intentionally held back pending explicit confirmation, since activating them makes real third-party API calls and real wallet debits possible. SMS-Man still has no row anywhere (no token supplied).
