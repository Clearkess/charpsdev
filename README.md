...
- Triggers on push to `main` when `mobile/**` changes, or manually via `workflow_dispatch` (with an optional checkbox to also build a Play-Store `.aab` bundle alongside the `.apk`).
- JDK 17 + Node 20 + Gradle cache, `npm ci` in `mobile/`, `npx cap sync android`, then `./gradlew assembleRelease`.
...
**Deployment status:** No live "deployment" step applies here in the usual sense — this doesn't touch Vercel or Railway, and there is nothing to "go live" on the web. The deliverable is the CI pipeline itself (`.github/workflows/android-build.yml`) plus the `mobile/` project it builds. Once pushed to `origin/main`, the workflow is immediately runnable from the GitHub Actions tab (or `gh workflow run android-build.yml`) to produce a downloadable, installable `app-release.apk` — verified locally in this session to build successfully with the identical Gradle command the workflow uses, per above.
