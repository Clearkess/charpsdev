...
**Frontend — ⚠️ blocked, not deployed.** This is a **recurring** blocker, not a new one — see §11's original "Vercel frontend deploy — blocked" note from Phase 2. The `VERCEL_TOKEN` available in this sandbox authenticates basic user identity (`GET /v2/user` succeeds) but every team/project-scoped call (`vercel deploy`, `GET /v6/deployments`, `vercel whoami --scope ...`) is rejected with `403 forbidden`, `"saml": true`, `"scope": "charps-dev"` — the `charps-dev` Vercel team enforces SSO that this personal-account token type cannot satisfy programmatically. This is a hard account/token-permission blocker, not a code issue; it was resolved once before (Phase 2 pass, §11) by the user supplying a fresh **team/project-scoped** token from the `charps-dev` team's own dashboard (Settings → Tokens), but that fix isn't persisted across sandbox sessions — a new sandbox session reverts to whatever `VERCEL_TOKEN` is preconfigured, which is the older personal-account token again.

**To unblock the frontend deploy**, one of:
1. Generate a new token from the `charps-dev` team's dashboard (Settings → Tokens, with the team as the token's owner/scope, not the personal account) and provide it for this session, or
2. Trigger the deploy manually from the Vercel dashboard (Deployments → Redeploy on the `charpsdev` project), or
3. Confirm/enable the Vercel GitHub integration's auto-deploy-on-push for `main` if it isn't firing already — commit `64e855e` has been on `origin/main` since this pass but no automatic Vercel deployment was observed to result from it.

The frontend has no changes in this phase anyway (Phase 10 was backend-only), so the live site at `https://charpsdev.vercel.app` continues serving whatever it was already serving (Phase 9's UI, if that deploy went through) — nothing regresses by leaving it as-is; the only thing "not yet live" is unrelated to this phase's actual changes.
